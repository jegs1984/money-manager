...
data class StagingRowUiState(
    val staging:            StagingTransactionEntity,
    val assignedCategoryId: Long? = null,
)

data class StagingReviewUiState(
    val rows:        List<StagingRowUiState>    = emptyList(),
...
@HiltViewModel
class StagingReviewViewModel @Inject constructor(
    private val repo: FinanceRepository,
) : ViewModel() {

    private val _periodId = MutableStateFlow<Long?>(null)
    private val _assignments = MutableStateFlow<Map<Long, Long>>(emptyMap())  // stagingId → categoryId
    private val _committed   = MutableStateFlow<Int?>(null)

    val uiState: StateFlow<StagingReviewUiState> = combine(
        _periodId.filterNotNull().flatMapLatest { pid ->
            combine(
                repo.observePendingStaging(),
        ...
        StagingReviewUiState(
            rows        = staging.map { s -> StagingRowUiState(s, assignments[s.id]) },
            categories  = cats,
            budgetItems = budgetItems,
            committed   = committed,
            loading     = false,
        )
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000), StagingReviewUiState())
    ...
    fun commit() = viewModelScope.launch {
        ...
        val n = repo.commitStagingBatch(entries, currentState.budgetItems)
        _committed.value = n
    }
}
