...
interface StagingTransactionDao {
    ...
    @Query("SELECT * FROM finance_staging_transaction WHERE is_processed = 0 ORDER BY original_date")
    ...
    @Query("UPDATE finance_staging_transaction SET assigned_category_id = :categoryId WHERE id = :id")
    suspend fun assignCategory(id: Long, categoryId: Long)

    @Query("UPDATE finance_staging_transaction SET is_processed = 1 WHERE id IN (:ids)")
    suspend fun markProcessed(ids: List<Long>)

    @Query("DELETE FROM finance_staging_transaction WHERE is_processed = 1")
